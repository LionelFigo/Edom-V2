...
<div class="p-8 bg-gray-50 min-h-screen">
    <div class="flex items-center gap-4 mb-10">
        ...
        <div>
            <h1 class="text-2xl font-bold text-black tracking-wide">Ubah Pertanyaan Evaluasi</h1>
            <p class="text-gray-600 text-sm mt-1">Kelola pertanyaan evaluasi dosen</p>
        </div>
    </div>

    <form action="{{ route('admin.pertanyaan.update', $pertanyaan->id) }}" method="POST" class="max-w-4xl">
        @csrf
        @method('PUT')

        <div class="grid grid-cols-[200px_1fr] items-start gap-4 mb-4">
            <label class="font-medium text-black mt-2">Isi Pertanyaan :</label>
            <textarea name="teks_pertanyaan" rows="4" required class="w-full border border-gray-300 rounded px-4 py-2 focus:outline-none focus:border-[#004684]">{{ $pertanyaan->teks_pertanyaan }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-[200px_1fr] items-start gap-4 mb-10">
            ...
            <select name="kategori_id" required class="w-full border border-gray-300 rounded px-4 py-2 focus:outline-none focus:border-[#004684] bg-white appearance-none">
                @foreach ($kategoris as $k)
                    <option value="{{ $k->id }}" {{ $pertanyaan->kategori_id == $k->id ? 'selected' : '' }}>
                        {{ $k->nama_kategori }}
                    </option>
                @endforeach
            </select>
