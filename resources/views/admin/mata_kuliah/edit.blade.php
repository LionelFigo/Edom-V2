...
<div class="p-8 bg-gray-50 min-h-screen">
    <div class="flex items-center gap-4 mb-10">
        ...
        <h1 class="text-2xl font-bold text-black tracking-wide">Ubah Mata Kuliah</h1>
    </div>

    <form action="{{ route('admin.mata_kuliah.update', $mataKuliah->id) }}" method="POST" class="max-w-3xl">
        @csrf
        @method('PUT')
        
        <div class="grid grid-cols-[200px_1fr] items-center gap-4 mb-4">
            <label class="font-medium text-black">Kode Mata Kuliah :</label>
            <input type="text" name="kode_mk" value="{{ $mataKuliah->kode_mk }}" required class="w-full border border-gray-300 rounded px-4 py-2 focus:outline-none focus:border-[#004684]">
        </div>

        <div class="grid grid-cols-[200px_1fr] items-center gap-4 mb-4">
            <label class="font-medium text-black">Nama Mata Kuliah :</label>
            <input type="text" name="nama_mk" value="{{ $mataKuliah->nama_mk }}" required class="w-full border border-gray-300 rounded px-4 py-2 focus:outline-none focus:border-[#004684]">
        </div>
